...
io.on('connection', (socket) => {
  ...
  socket.on('connect-to-user', (data: { targetUserId: string }) => {
    const userId = socket.data.userId;
  ...
  socket.on('voice-offer', (data: { offer: any, to: string }) => {
    const userId = socket.data.userId;
    const targetUserInfo = activeUsers.get(data.to);
  ...
  socket.on('voice-answer', (data: { answer: any, to: string }) => {
    const userId = socket.data.userId;
    const targetUserInfo = activeUsers.get(data.to);
...
function matchUsers() {
  ...
  for (let i = 0; i < queueCopy.length; i++) {
    ...
    if (!user1 || !user1.userId || !activeUsers.has(user1.userId)) continue;
    
    for (let j = i + 1; j < queueCopy.length; j++) {
      const user2 = queueCopy[j];
      
      // Skip invalid entries
      if (!user2 || !user2.userId || !activeUsers.has(user2.userId)) continue;
      ...
      if (areUsersCompatible(user1, user2)) {
        ...
        userQueue = userQueue.filter(user => 
        ...
        if (user1Info && user2Info) {
          ...
          if (socket1) socket1.data.partnerId = user2.userId;
          if (socket2) socket2.data.partnerId = user1.userId;
          ...
          const getFlag = (country: string) => {
            if (country === 'unknown' || country === 'any') return '🌍';
            // If the country code is 2 letters, convert to regional indicator symbols
            if (/^[a-z]{2}$/i.test(country)) {
              return String.fromCodePoint(
                ...country.toLowerCase().split('').map(c => c.charCodeAt(0) + 127397)
              );
            }
            return '🌍';
          };
...
function areUsersCompatible(user1: QueueUser, user2: QueueUser): boolean {
  ...
  if (
    ...
    user1Prefs.blockedCountries.some((c: string) => 
      user2Prefs.preferredCountries.includes(c)
    )
  ) {
    return false;
  }
  ...
  if (
    ...
    user2Prefs.blockedCountries.some((c: string) => 
      user1Prefs.preferredCountries.includes(c)
    )
  ) {
    return false;
  }
  ...
  if (
    ...
    const hasCommonCountry = user1Prefs.preferredCountries.some(
      (c: string) => user2Prefs.preferredCountries.includes(c)
    );
  ...
  if (
    user1Prefs.interests?.length &&
    user2Prefs.interests?.length
  ) {
    // Check if they have at least one common interest
    const hasCommonInterest = user1Prefs.interests.some(
      (interest: string) => {
        // Case insensitive match
        const lowerCaseInterest = interest.toLowerCase();
        return user2Prefs.interests.some(
          (i2: string) => i2.toLowerCase() === lowerCaseInterest
        );
      }
    );
