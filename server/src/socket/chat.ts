...
// تعريف نوع البيانات لرسالة الدردشة
export interface ChatMessage {
  id: string;         // معرف فريد للرسالة
  content: string;    // محتوى الرسالة
  sender: string;     // مرسل الرسالة
  timestamp: number;  // وقت الإرسال
  to: string;         // المستلم
  signature?: string; // توقيع الرسالة للتحقق من الصحة
}

// تخزين مؤقت للرسائل المرسلة لضمان التسليم
const pendingMessages = new Map<string, { message: ChatMessage, attempts: number, timestamp: number }>();
...
function validateMessage(message: any): boolean {
  ...
  if (!message || !message.id) {
    if (DEBUG) console.log(`Invalid message: Missing ID`, message);
    return false;
  }
  ...
  if (!messageContent) {
    if (DEBUG) console.log(`Invalid message: Missing content`, message);
    return false;
  }
  
  if (!message.to) {
    if (DEBUG) console.log(`Invalid message: Missing recipient`, message);
    return false;
  }
  
  // التحقق من طول الرسالة
  if (messageContent.length > MAX_MESSAGE_LENGTH) {
    if (DEBUG) console.log(`Invalid message: Text too long (${messageContent.length} chars)`);
    return false;
  }
  ...
  if (!uuidRegex.test(message.id)) {
    if (DEBUG) console.log(`Invalid message: Invalid UUID format: ${message.id}`);
    return false;
  }
 ...
 */
function createMessageSignature(message: { id: string; text: string; from: string; timestamp: number }): string {
  const data = `${message.id}:${message.text}:${message.from}:${message.timestamp}`;
  return crypto.createHmac('sha256', MESSAGE_SECRET).update(data).digest('hex');
}

/**
 * التحقق من توقيع الرسالة
 */
function verifyMessageSignature(message: { id: string; text: string; from: string; timestamp: number }, signature: string): boolean {
...
export const setupChatEvents = (io: Server, socket: Socket) => {
  ...
  // إرسال رسالة دردشة
  socket.on('chat-message', (message: { id: string; text?: string; message?: string; to: string; timestamp: number }, callback) => {
    try {
      ...
      const recipientInfo = userService.getUserInfo(normalizedMessage.to);
      
      if (DEBUG) console.log(`[ChatServer] Recipient info for ${normalizedMessage.to}:`, recipientInfo ? 'Found' : 'Not found');
      ...
      if (!recipientInfo) {
        if (DEBUG) console.log(`[ChatServer] Recipient ${normalizedMessage.to} not found or offline, message queued for later delivery`);
  ...
  socket.on('message-received', (data: { id: string; to: string; signature?: string }) => {
    try {
      const userId = socket.data.userId;
      if (!userId) {
        console.error('User ID not found in socket data');
      ...
      if (DEBUG) console.log(`[ChatServer] Message receipt confirmation from ${userId} for message ${data.id}`);
      
      // إزالة الرسالة من التخزين المؤقت لأنها تم استلامها
      pendingMessages.delete(data.id);
      
      // إرسال تأكيد الاستلام إلى المرسل الأصلي
      const senderInfo = userService.getUserInfo(data.to);
      if (senderInfo) {
        if (DEBUG) console.log(`[ChatServer] Forwarding receipt to sender ${data.to} via socket ${senderInfo.socketId}`);
        io.to(senderInfo.socketId).emit('message-received', {
          id: data.id,
          from: userId
        });
      } else {
        if (DEBUG) console.log(`[ChatServer] Sender ${data.to} not found, cannot forward receipt`);
      }
    } catch (error) {
  ...
  socket.on('message-read', (data: { id: string; to: string }) => {
    try {
      const userId = socket.data.userId;
      if (!userId) {
        console.error('User ID not found in socket data');
      ...
      if (DEBUG) console.log(`[ChatServer] Message read confirmation from ${userId} for message ${data.id}`);
      
      // إرسال تأكيد القراءة إلى المرسل الأصلي
      const senderInfo = userService.getUserInfo(data.to);
      if (senderInfo) {
        if (DEBUG) console.log(`[ChatServer] Forwarding read receipt to sender ${data.to} via socket ${senderInfo.socketId}`);
        io.to(senderInfo.socketId).emit('message-read', {
          id: data.id,
          from: userId
        });
      } else {
        if (DEBUG) console.log(`[ChatServer] Sender ${data.to} not found, cannot forward read receipt`);
      }
    } catch (error) {
  ...
  socket.on('typing', (data: { to: string; isTyping: boolean }) => {
    try {
      ...
      const recipientInfo = userService.getUserInfo(data.to);
      
      if (!recipientInfo) {
        if (DEBUG) console.log(`[ChatServer] Typing recipient ${data.to} not found`);
        return;
      }
      
      if (DEBUG && data.isTyping) console.log(`[ChatServer] User ${userId} is typing to ${data.to}`);
  ...
  socket.on('get-pending-messages', () => {
    try {
      ...
      const pendingForUser: ChatMessage[] = [];
...
export const retryPendingMessages = (io: Server) => {
  const now = Date.now();
  ...
  pendingMessages.forEach((item, messageId) => {
    ...
    if (now - item.timestamp >= RETRY_INTERVAL) {
      ...
      const recipientInfo = userService.getUserInfo(item.message.to);
      if (!recipientInfo) {
        if (DEBUG) console.log(`Recipient ${item.message.to} still not available, will retry later (attempt ${item.attempts})`);
        return;
      }
      
      if (DEBUG) console.log(`Retrying message ${messageId} delivery to ${item.message.to} via socket ${recipientInfo.socketId}, attempt ${item.attempts}`);
...
export const cleanupOldPendingMessages = () => {
  const now = Date.now();
...
export default (io: Server) => {
  io.on('connection', (socket: Socket) => {
    ...
    const userId = socket.data.userId;
    if (userId) {
      if (DEBUG) console.log(`[ChatServer] User ${userId} connected, checking for pending messages`);
