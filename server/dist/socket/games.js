...
const gameSchemas_1 = require("../validation/gameSchemas");
...
const setupGameEvents = (io, socket) => {
    ...
    gameController_1.gameController.setSocketServer(io);
    ErrorHandler_1.errorHandler.setSocketServer(io);
    /**
    ...
    socket.on('game-join-room', (data) => {
        try {
            ...
            const validationResult = (0, validator_1.validate)(gameSchemas_1.gameRoomJoinSchema, data);
            ...
            const targetUserInfo = userService_1.userService.getUserInfo(validData.to);
            if (!targetUserInfo) {
                socket.emit('game-error', {
            ...
            if (validData.isFirstPlayer) {
                ...
                const game = gameController_1.gameController.createGameRoom(validData.roomId, validData.gameType, socket.data.userId);
                if (!game) {
                    socket.emit('game-error', {
                ...
                const game = gameController_1.gameController.joinGameRoom(validData.roomId, socket.data.userId);
                if (!game) {
                    socket.emit('game-error', {
                ...
                const roomCreatorInfo = userService_1.userService.getUserInfo(roomCreator);
                if (roomCreatorInfo) {
     ...
     */
    socket.on('game-invite-timeout', (data) => {
        try {
            const targetUserInfo = userService_1.userService.getUserInfo(data.to);
            if (!targetUserInfo) {
    ...
    socket.on('game-join', (data) => {
        try {
            const targetUserInfo = userService_1.userService.getUserInfo(data.partnerId);
            if (!targetUserInfo) {
                socket.emit('game-error', {
                    message: "Could not find your partner. They may have disconnected.",
                    from: 'system'
            ...
            // Create a new game room
            const game = gameController_1.gameController.createGameRoom(roomId, data.gameType, socket.data.userId);
            if (!game) {
                socket.emit('game-error', {
                    message: "Failed to create game room.",
                    from: 'system'
    ...
    socket.on('game-leave-room', (data) => {
        try {
            ...
            const success = gameController_1.gameController.leaveGameRoom(data.roomId, socket.data.userId);
    ...
    socket.on('game-move', (data) => {
        try {
            ...
            switch (data.gameType) {
                case 'tic-tac-toe':
                    validationResult = (0, validator_1.validate)(gameSchemas_1.ticTacToeMoveSchema, data);
                    break;
                case 'rock-paper-scissors':
                    validationResult = (0, validator_1.validate)(gameSchemas_1.rockPaperScissorsMoveSchema, data);
                    break;
                case 'word-galaxy':
                    validationResult = (0, validator_1.validate)(gameSchemas_1.wordGalaxyMoveSchema, data);
                    break;
                default:
                    // استخدام معالج الأخطاء الجديد
                    ErrorHandler_1.errorHandler.sendErrorToUser(socket.data.userId, socket.id, ErrorHandler_1.errorHandler.createError(ErrorHandler_1.ErrorType.VALIDATION, 'GAME_TYPE_UNSUPPORTED', `Unsupported game type: ${data.gameType}`, { gameType: data.gameType }));
                    return;
            }
            if (!validationResult.success) {
                // استخدام معالج الأخطاء الجديد
                ErrorHandler_1.errorHandler.sendErrorToUser(socket.data.userId, socket.id, ErrorHandler_1.errorHandler.createError(ErrorHandler_1.ErrorType.VALIDATION, 'INVALID_MOVE_DATA', `Invalid move data: ${validationResult.error}`, { gameType: data.gameType, error: validationResult.error }));
                return;
            }
            // استخدام البيانات المتحقق منها
            const validData = validationResult.data;
            ...
            if (!targetUserInfo) {
                // استخدام معالج الأخطاء الجديد
                ErrorHandler_1.errorHandler.sendErrorToUser(socket.data.userId, socket.id, ErrorHandler_1.errorHandler.createError(ErrorHandler_1.ErrorType.CONNECTION, 'PARTNER_NOT_FOUND', "Could not find your partner. They may have disconnected.", { targetId: validData.to }));
                return;
            }
            // Process move on the server side
            if (validData.roomId) {
                const result = gameController_1.gameController.processMove(validData.roomId, socket.data.userId, validData.move);
                if (!result) {
                    // استخدام معالج الأخطاء الجديد
                    ErrorHandler_1.errorHandler.sendErrorToUser(socket.data.userId, socket.id, ErrorHandler_1.errorHandler.createError(ErrorHandler_1.ErrorType.GAME_LOGIC, 'INVALID_MOVE', "Invalid move or game not found.", {
                        roomId: validData.roomId,
                        gameType: validData.gameType,
                        move: validData.move
                    }));
            ...
            // استخدام معالج الأخطاء الجديد
            ErrorHandler_1.errorHandler.sendErrorToUser(socket.data.userId, socket.id, ErrorHandler_1.errorHandler.createError(ErrorHandler_1.ErrorType.SERVER, 'MOVE_PROCESSING_ERROR', "Failed to process game move due to server error.", { error: error.message }));
        }
    });
    /**
     * Handle game reset
    ...
    socket.on('game-reset', (data) => {
        try {
            ...
            const validationResult = (0, validator_1.validate)(gameSchemas_1.gameResetSchema, data);
            if (!validationResult.success) {
                socket.emit('game-error', {
                    message: `Invalid data: ${validationResult.error}`,
                    from: 'system',
            ...
            if (validData.roomId) {
                const success = gameController_1.gameController.resetGame(validData.roomId, socket.data.userId);
                if (!success) {
                    socket.emit('game-error', {
    ...
    socket.on('disconnecting', () => {
        try {
            ...
            const allRooms = gameController_1.gameController.getAllActiveRooms();
            ...
            for (const [roomId, game] of allRooms.entries()) {
                ...
                if (gameState.players.includes(socket.data.userId)) {
                    // Notify the partner
                    const partnerId = gameState.players.find((id) => id !== socket.data.userId);
                    if (partnerId) {
                        const partnerInfo = userService_1.userService.getUserInfo(partnerId);
                        if (partnerInfo) {
                    ...
                    // Remove player from room
                    gameController_1.gameController.leaveGameRoom(roomId, socket.data.userId);
                }
            }
        }
        catch (error) {
    ...
    socket.on('game-reconnect', (data) => {
        try {
            ...
            const gameState = gameController_1.gameController.handleReconnection(data.roomId, socket.data.userId, socket.id);
            if (!gameState) {
                socket.emit('game-error', {
            ...
            const otherPlayers = gameState.players.filter((id) => id !== socket.data.userId);
            for (const playerId of otherPlayers) {
                const playerInfo = userService_1.userService.getUserInfo(playerId);
                if (playerInfo) {
    ...
    socket.on('game-disconnect', (data) => {
        try {
            ...
            if (!userId || !data.roomId)
                return;
            const game = gameController_1.gameController.getGameByRoom(data.roomId);
            if (!game)
                return;
            ...
            if (players.includes(userId)) {
                ...
                if (data.temporary) {
                    ...
                    const otherPlayer = players.find((id) => id !== userId);
                    if (otherPlayer) {
                        const otherPlayerInfo = userService_1.userService.getUserInfo(otherPlayer);
                        if (otherPlayerInfo) {
                    ...
                    gameController_1.gameController.leaveGameRoom(data.roomId, userId);
                }
            }
        }
        catch (error) {
    ...
    socket.on('disconnect', () => {
        ...
        if (!userId)
            return;
        const rooms = gameController_1.gameController.getAllActiveRooms();
        for (const [roomId, game] of rooms.entries()) {
            ...
            if (players.includes(userId)) {
                ...
                gameService_1.gameService.recordDisconnection(userId);
            }
        }
    });
    /**
